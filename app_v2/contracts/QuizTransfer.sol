...
contract QuizTransfer {
    ...
    // 퀴즈에 대한 코인 차감 거래 발생
    function setStudyQuizTransfer(bytes32 _senderId,  bytes32 _senderName, bytes32 _receiverid, bytes32 _receiverName, uint _coin, bytes32 _date, uint _study_id, address _receiverAddress, bytes32 _idx_hash) public payable {
        studyQuizTransfer memory transferItem 
        = studyQuizTransfer(_senderId,_senderName, _receiverid, _receiverName, _coin, _date, _idx_hash); 
